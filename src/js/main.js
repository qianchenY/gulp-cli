    ...
    $.fn.extend({
        clientAnimate: function(options) {
            ...
            function checkInitialVisibility() {
                var shouldAnimate = windowWidth >= settings.animatStop;
            ...
            function handleScroll() {
                ...
                var shouldToggle = settings.toggle && windowWidth >= settings.animatStop;
                
                elementsArray.forEach(function(element) {
                    var $el = $(element);
                    var rect = element.getBoundingClientRect();
                    var hasClass = $el.hasClass(settings.className);
                    
                    // 检查元素是否在视口中
                    var isInViewport = rect.bottom > difference && 
                                     rect.top < windowHeight - difference;
                    
                    var isOutOfViewport = rect.bottom < -difference || rect.top > windowHeight + difference;
                    
                    if (isInViewport) {
                        if (!hasClass) {
                            $el.addClass(settings.className);
                        }
                    } else if (isOutOfViewport && shouldToggle && hasClass) {
            ...
            // 使用节流函数绑定滚动事件
            scrollHandler = throttle(handleScroll, 16); // 约60fps
            
            // 执行初始检查
            checkInitialVisibility();
            
            // 绑定滚动事件
            $(window).on('scroll.clientAnimate', scrollHandler);
